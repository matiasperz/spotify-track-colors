...
class ImageColorExtractor {
  ...
  extractFromUrl = async (url, colorCount = DEFAULT_COLOR_COUNT) => {
  ...
  extractColors = async (imageDir, colorCount = DEFAULT_COLOR_COUNT) => {
    const rgbColors = await colorThief.getPalette(imageDir, colorCount)
    const hexColors = rgbColors.map((rgb) => onecolor(`rgb(${rgb[0]}, ${rgb[1]}, ${rgb[2]})`).hex())

    return hexColors
  }

  downloadImage = (url, callback) => new Promise((resolve, reject) => {
    fetch(url)
    .then(res => 
      res.body.pipe(
